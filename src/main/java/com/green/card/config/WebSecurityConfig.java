...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter { // 2
    ...
    protected void configure(HttpSecurity http) throws Exception { // 5
        http
                .authorizeRequests() // 6
                .antMatchers("/groovy").permitAll() // 누구나 접근 허용
                .antMatchers("/groovy").hasRole("USER") // USER, ADMIN만 접근 가능
                .antMatchers("/admin").hasRole("ADMIN") // ADMIN만 접근 가능
                .anyRequest().authenticated() // 나머지 요청들은 권한의 종류에 상관 없이 권한이 있어야 접근 가능
                .and()
                .formLogin() // 7
                .loginPage("/groovy") // 로그인 페이지 링크
                .defaultSuccessUrl("/home") // 로그인 성공 후 리다이렉트 주소
                .and()
                .logout() // 8
                .logoutSuccessUrl("/groovy") // 로그아웃 성공시 리다이렉트 주소
